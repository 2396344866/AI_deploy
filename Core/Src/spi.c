...
void MX_SPI1_Init(void)
{
  ...
  /* 提速：SPI123 内核时钟 160MHz，默认 /128 时 SCK≈1.25MHz（仅发挥 ~1.5% 带宽）。
     改为 /4 -> 40MHz，处于 W25Q64BV 读上限内（0x03 读≤50MHz，0x0B 快读≤80MHz）。
     若改用 Fast Read(0x0B)+8 哑元时钟，可进一步上 /2=80MHz。
     已在 .ioc 同步改为 SPI_BAUDRATEPRESCALER_4，防止 CubeMX 重新生成时被覆盖。 */
  hspi1.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_4;
  if (HAL_SPI_Init(&hspi1) != HAL_OK)
  {
    Error_Handler();
...
void HAL_SPI_MspInit(SPI_HandleTypeDef* spiHandle)
{
  ...
  */
  ...
  /* 提速配套：SCK 提到 40MHz 级别时，必须把 IO 翻转速率拉到 VERY_HIGH，否则边沿塌陷导致采样错误 */
...
// RX DMA 完成回调：与 TX 共用同一个信号量（SPI 操作串行，不会冲突）
