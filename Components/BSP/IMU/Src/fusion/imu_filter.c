...
static int16_t s_win[6][TRIM_WIN_MAX];   /* [0..2]=accel, [3..5]=gyro，滑动窗口 */
static uint8_t s_widx   = 0;             /* 环形写指针 */
static uint8_t s_wcount = 0;             /* 已填充样本数（窗口未满时小于 trim_win） */
...
static int16_t trim_mean(const int16_t *w, uint8_t n)
{
    ...
    return (int16_t)((sum - min - max) / (int16_t)(n - 2));
}
...
void ImuFilter_Update(const int16_t raw_accel[3], const int16_t raw_gyro[3], ImuData_t *out)
{
    ...
    if (g_imu_filter.trim_en) {
        ...
        uint8_t n = (s_wcount < w) ? s_wcount : w;   /* 窗口未满时用已填充数 */
        for (int i = 0; i < 6; i++) {
            s_win[i][s_widx] = src[i];
            if (n >= 1) src[i] = trim_mean(s_win[i], n);
        }
        s_widx = (s_widx + 1U) % w;
        if (s_wcount < w) s_wcount++;
    }
