 ...
 *   协议输出。固定 59 通道宽帧，按宏开关分组（见 dbg_config.h）。
 *
 ...
 *   DBG_UART_BAUD 覆盖式重设，详见 Components/Debug/Ref/motor_vofa_telemetry.md。
 * ============================================================================= */
...
/* 打印通道清单（控制台 `chan` 命令调用，供 VOFA 绑通道时对照）。
 *   仅 TRACE + 遥测开(DBG_TELEMETRY_ENABLE==1) 才编进；非 TRACE 由 freertos.c 落提示。
 *   grp=NULL/"" 打印全部；否则按组名(不区分大小写)过滤：ACC/GYRO/MAG/ATT/GBIAS/CTRL。 */
