...
> 原 `Doc/Keil_MDK_ARM_工程排错记录.md` 的 **问题 2 / 问题 3 / 问题 4** 迁入此处。
...
- **根因（致命）**：uvprojx 换行符被改成 **LF**，而 **Keil 强制要求 CRLF**。`ET.fromstring` 能解析 LF（误报合法），Keil 直接拒绝→工程读成空壳。此前 AI 的 LF→CRLF 脚本因 shell 转义吞掉 `\r` **实际未生效**，验证代码也误报成功。
