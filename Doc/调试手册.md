...
| `APP_PROFILE_DIAG_INFERENCE` | Inference, Sensor, Logger |
| `APP_PROFILE_OBJDET_INFERENCE` | Esp32S3, Sensor, Logger |
...
> 📌 **L1 正交门控（2026-08-30）**：`DBG_LOG_<TASK>` 还受 `APP_ENABLE_X`（app_config.h，由 `APP_PROFILE_*` 派生）正交门控——**仅当对应模块编进固件（`APP_ENABLE_X=1`）时才为 1，否则强制 0**，即使 `DBG_LOG_ENABLE=1`。故下表"定义值"是动态结果：当前 `APP_PROFILE_FLASH/SCREEN/ESP32S3` 下 `DBG_LOG_SENSOR/SCREEN/FLASH/ESP32S3/LOGGER/POSTEST=1`，`DBG_LOG_DIAG/MOTOR/NET=0`；切 profile 重启编译即随之变化，无需手改本表。

| 宏 | 定义值 | 实际生效条件 | 作用 |
|---|---|---|---|
| `DBG_LOG_ENABLE` | 1 | 运行级≥DEBUG 才编入（默认 INFO→0） | 任务级 DEBUG/TRACE 文本总闸 |
| `DBG_LOG_DIAG` | 0 | 同上 | DIAG 模块内部 detail（POST 内 ML 自检 init/sample/metrics 由本开关门控，非 POST 框架） |
| `DBG_LOG_MOTOR` | 0 | 同上 | 电机环文本 |
| `DBG_LOG_NET` | 0 | 同上 | 网络诊断文本：① DEBUG 级 = 连接各 AT 步骤进入日志(AT/CWMODE/ATE0/CWJAP/CIPSTART/CIPMODE/CIPSEND/CONNACK) + 失败时回采模块真实响应(`+CWJAP:n`/`FAIL`/CONNACK rc)；② TRACE 级(`LOG_COMPILE_MAX_LEVEL>=TRACE` 才编进) = 状态机 `connect failed: init/tcp/mqtt` 每步返回码 |
| `DBG_LOG_SENSOR` | 0 | 同上 | 姿态/滤波文本 |
| `DBG_LOG_SCREEN` | 0 | 同上 | 屏显文本 |
| `DBG_LOG_FLASH` | 0 | 同上 | Flash 模块内部 detail（POST 内 Flash_Test step 进度由本开关门控，非 POST 框架） |
| `DBG_LOG_LOGGER` | 0 | 同上 | 日志任务自身文本 |
| `DBG_LOG_ESP32S3` | 0 | 同上 | 图像结论帧文本 |
| `DBG_LOG_POSTEST` | 1 | 同上 | POSTEST 探针（定位卡死/复位环） |
...
| `DBG_FRAME_N` | 59 | ≥TRACE | 固定帧宽（**勿改**，须与 `dbg_telemetry.c`/motor_vofa_telemetry.md 一致） |
| `DBG_TELEMETRY_DECIMATE` | 2 | ≥TRACE | 遥测降采样（每 N 拍发 1 帧；59ch@200Hz≈95KB/s 超 921600 波特上限，降 100Hz≈47KB/s 留余量） |
...
> **完整清单见 `Components/Debug/Ref/uart_list.md`**（唯一真相源）。本节只留高频速查与坑。

**双轨命名**：旧单键全部保留，`域.动作` 长名并存，二者等价（长名在入口原地归一化为旧键）。
⚠ `S` 急停永远单键。⚠ 大小写敏感（`M`=磁力计，`m`=电机域 `mot`）。⚠ 帧上限 **63 B**。

**三条公约**：①无参=查询 ②未知命令报错（不再静默回显）③改值即回读。

| 高频 | 长名 | 作用 |
|---|---|---|
| `debug4` | `sys.lvl 4` | 解静音看回执（**TRACE 下 `LOG_I` 被吞，无输出通常=成功**） |
| `debug5` | `sys.lvl 5` | TRACE，VOFA 59 通道全开 |
| `C` | `att.cal` | 陀螺零偏静态标定（板子须完全静止） |
| `P` | `att.ref` | `P`=查 ref/err；`P deg`=设 pitch；`P r,p,y`=三轴 |
| `K` | `att.pid` | `K`=查增益；`K kp,ki,kd`=设（⚠ 无参不再清零） |
| `F` | `filt` | 滤波配置，`f lag/trim/elag/mlag/mtrim 1/0` |
| `M` | `mag` | `M`=状态；`mag init`；`mag.align y,p,r` |
| `D` | `att.dump` | MPU6050 寄存器 dump |
| `H` | `att.yaw` | 航向保持增益 |
| `A`/`B`/`S`/`R` | `mot.a`/`mot.b`/`S`/`mot.run` | 电机；`S`=急停 |
| `?` | `help` | 域索引 |

> 调参顺序建议：磁航向噪声大时先 `filt mlag 1`/`filt mtrim 1` 加强航向滤波，再发 `C` 标定陀螺；
> 若 `C` 反复 `CAL FAIL: I2C read only X/100`，优先查 MPU6050 的 I2C 接线（杜邦线接触不良会掉包，标不定零偏）。
...
| 故障诊断推理 | `APP_PROFILE_DIAG_INFERENCE` | Inference, Sensor, Logger |
| 目标检测推理 | `APP_PROFILE_OBJDET_INFERENCE` | Esp32S3, Sensor, Logger |
...
9. 出 bug 先查 `Components/Debug/Error/Error_Readme_idx.md`（运行期，按任务 E 事件）+ 编译/链接查 `compile_link_error.md` + uvprojx 查 `uvprojx_error.md`。
10. **遥测帧宽铁律**：`DBG_FRAME_N` 必 = `dbg_telemetry.c` 枚举 `CH_COUNT`（`_Static_assert` 编译期守）；四文件须同步——`dbg_config.h` / `dbg_telemetry.c` / `Components/Debug/Ref/motor_vofa_telemetry.md` / `Components/Debug/Ref/vofa_panel.json`；改通道布局后必跑 `Components/Debug/Tools/verify_dbg_frame.py` 四同步校验，全 PASS 才提交。标准操作步骤见 `motor_vofa_telemetry.md` 末尾「通道变更标准操作步骤（SOP）」。
